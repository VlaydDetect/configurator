...
pub fn check(input: model::Input) -> syn::Result<model::Validate> {
	...
	{
		error.maybe_fold(syn::Error::new(span, "transparent structs must have exactly one field"));
	}
...
fn is_unary_struct(k: &model::ValidateKind) -> bool {
	match k {
		...
		model::ValidateKind::Struct(model::ValidateVariant::Struct(fields)) => {
			fields.iter().filter(|(_, field)| field.skip.is_none()).count() == 1
		}
		_ => false,
	}
}
...
fn check_field(field: model::Field, options: &model::Options) -> syn::Result<model::ValidateField> {
	...
	if let Some(span) = field.skip && !field.is_empty() {
		error.maybe_fold(syn::Error::new(span, "`skip` may not be combined with other rules"))
	}

	if let Some((span, _)) = field.dive && field.rule_set.inner.is_some() {
		error.maybe_fold(syn::Error::new(span, "`dive` may not be combined with `inner`"))
	}
	...
	T: PartialOrd,
{
	if let Some(equal) = range.equal {
		return if range.min.is_some() || range.max.is_some() {
			Err(syn::Error::new(range.span, "no `min` or `max` allowed if using `equal`"))
		} else {
	...
	match (range.min, range.max) {
		...
		(Some(_), Some(_)) => {
			Err(syn::Error::new(range.span, "`min` must be lower than or equal to `max`"))
		}
...
fn check_regex(value: model::Pattern) -> syn::Result<model::ValidatePattern> {
	match value {
		model::Pattern::Lit(lit) => {
			...
			Err(syn::Error::new(lit.span, "regex feature must be enabled to use literal patterns"))
		}
