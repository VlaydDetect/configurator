...
/// ```
/// use std::path::Path;
/// use configurator::config::util::diff_paths;
///
...
/// }
///
/// let c0: Config = Configurator::from(("cli_colors", "true")).extract().unwrap();
/// let c1: Config = Configurator::from(("cli_colors", "TRUE")).extract().unwrap();
/// let c2: Config = Configurator::from(("cli_colors", 1)).extract().unwrap();
/// assert_eq!(c0.cli_colors, true);
/// assert_eq!(c1.cli_colors, true);
/// assert_eq!(c2.cli_colors, true);
///
/// let c0: Config = Configurator::from(("cli_colors", "false")).extract().unwrap();
/// let c1: Config = Configurator::from(("cli_colors", "fAlSe")).extract().unwrap();
/// let c2: Config = Configurator::from(("cli_colors", 0)).extract().unwrap();
...
///
/// ```
/// use configurator::{Configurator, map};
/// use serde::{Serialize, Deserialize};
///
/// #[derive(Debug, Clone, Serialize, Deserialize)]
/// pub struct Config {
///     #[serde(with = "configurator::config::util::vec_tuple_map")]
///     pairs: Vec<(String, usize)>
/// }
///
/// let map = map!["key" => 1, "value" => 100, "name" => 20];
/// let c: Config = Configurator::from(("pairs", map)).extract().unwrap();
...
pub mod vec_tuple_map {
    ...
    {
        se.collect_map(vec.iter().map(|(k, v)| (k, v)))
    }
...
use crate::config::value::{Value, Dict};
...
///
/// ```rust
/// use configurator::{config::util::nest, config::value::Value};
///
...
macro_rules! cloneable_fn_trait {
    ($Name:ident: $($rest:tt)*) => {
        ...
        $crate::config::util::make_cloneable!($Name: Cloneable);
    }
}
