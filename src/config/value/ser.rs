...
use crate::config::error::{Error, Kind};
use crate::config::value::{Dict, Empty, Num, Value};
...
impl Serializer for ValueSerializer {
	...
	fn serialize_newtype_variant<T: Serialize + ?Sized>(
		...
		Ok(crate::config::util::map![variant => value.serialize(self)?].into())
	}
...
impl ser::SerializeSeq for SeqSerializer {
	...
	fn end(self) -> Result<Self::Ok> {
		...
		match self.tag {
			Some(tag) => Ok(crate::config::util::map![tag => value].into()),
			None => Ok(value),
		}
	}
}
