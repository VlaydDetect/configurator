...
use crate::config::value::{Tag, ValueSerializer, magic::Either};
use crate::config::error::{Error, Actual};
...
impl Value {
    ...
    ///
    /// ```
    /// use configurator::value::{Value, Empty};
    ///
    ...
    ///
    /// ```rust
    /// use configurator::{value::Value, config::util::map};
    ///
    ...
    ///
    /// ```
    /// use configurator::{Configurator, Profile, value::Value, config::util::map};
    ///
    /// let map: Value = Configurator::from(("key", "value")).extract().unwrap();
    /// let value = map.find_ref("key").expect("value");
    /// assert_eq!(value.as_str(), Some("value"));
    /// assert!(!value.tag().is_default());
    /// assert_eq!(value.tag().profile(), Some(Profile::Global));
    ///
    /// let map: Value = Configurator::from(("key", map!["key2" => 123])).extract().unwrap();
    ...
    ///
    /// ```
    /// use configurator::value::{Value, Num};
    ///
    ...
    ///
    /// ```rust
    /// use configurator::{value::Value, config::error::Actual};
    ///
...
impl Num {
    ...
    ///
    /// ```rust
    /// use configurator::{value::Num, config::error::Actual};
    ///
...
impl Empty {
    ...
    ///
    /// ```rust
    /// use configurator::{value::Empty, config::error::Actual};
    ///
