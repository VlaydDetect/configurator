...
use crate::config::error::{Error, Kind, Result};
use super::{Value, Num, Empty, Dict, Tag};
...
impl<'c, I: Interpreter> ConfiguredValueDe<'c, I> {
    pub fn from(config: &'c Configurator, value: &'c Value) -> Self {
...
macro_rules! apply_then_forward_to_deserialize_any {
    ( $( $($f:ident),+ => |$this:tt| $apply:expr, $errmap:expr),* $(,)? ) => {
        $(
            $(
...
impl<'de> Visitor<'de> for ValueVisitor {
    ...
    fn visit_enum<A: de::EnumAccess<'de>>(self, data: A) -> result::Result<Self::Value, A::Error> {
        ...
        Ok(crate::config::util::nest(&tag, variant.newtype_variant()?))
    }
