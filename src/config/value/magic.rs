...
/// use configurator::{Configurator, value::magic::RelativePathBuf, Jail};
/// use configurator::providers::{Env, Format, Toml, Serialized};
///
...
///     let c: Config = Configurator::from(Toml::file("Config.toml")).extract()?;
...
///     let c: Config = Configurator::from(Serialized::defaults(&c)).extract()?;
///     assert_eq!(c.path.original(), Path::new("a/b/c.html"));
///     assert_eq!(c.path.relative(), jail.directory().join("a/b/c.html"));
///     assert_ne!(c.path.relative(), Path::new("a/b/c.html"));
///
...
///     let c: Config = Configurator::from(Toml::file("Config.toml"))
...
///     let configurator = Configurator::from(Serialized::defaults(Config {
///         path: "some/default/path".into()
///     }));
///
///     let default: Config = configurator.extract()?;
...
///     let overriden: Config = configurator.merge(Toml::file("Config.toml")).extract()?;
...
/// use serde::{Deserialize, Serialize};
/// use configurator::{Configurator, value::magic::RelativePathBuf, Jail};
/// use configurator::providers::{Format, Toml, Serialized};
///
...
///     "#)?;
///
///     // Create a configurator with a serialize `Config`.
///     let configurator = Configurator::from(Toml::file("Config.toml"));
///     let config = configurator.extract::<Config>()?;
///     let configurator = Configurator::from(Serialized::defaults(config));
///
///     // This fails, as expected.
///     let relative = configurator.extract_inner::<PathBuf>("relative");
///     assert!(relative.is_err());
///
///     // These succeed. This one uses the originally written path.
///     let root = configurator.extract_inner::<PathBuf>("root")?;
///     assert_eq!(root, PathBuf::from("root/path"));
///
///     // This one the magic relative path.
///     let temp = configurator.extract_inner::<PathBuf>("temp")?;
...
impl Magic for RelativePathBuf {
	...
	fn deserialize_from<'de: 'c, 'c, V: de::Visitor<'de>, I: Interpreter>(
		...
		if let Some(d) = de.value.as_dict()
			&& let Some(mpv) = d.get(Self::FIELDS[0])
			&& mpv.to_empty().is_none()
		{
		...
		let mut map = crate::config::value::Map::new();
...
impl RelativePathBuf {
	...
	///
	/// use configurator::{Configurator, value::magic::RelativePathBuf, Jail};
	/// use configurator::providers::{Format, Toml};
	///
	...
	/// # let config: Config = Configurator::from(Toml::file("Config.toml")).extract()?;
	...
	///
	/// use configurator::{Configurator, value::magic::RelativePathBuf, Jail};
	/// use configurator::providers::{Env, Format, Toml};
	///
	/// #[derive(Debug, PartialEq, serde::Deserialize)]
	/// struct Config {
	///     path: RelativePathBuf,
	...
	///     let c: Config = Configurator::from(Env::raw().only(&["PATH"])).extract()?;
	...
	/// ```rust
	/// use std::path::PathBuf;
	/// use configurator::value::magic::RelativePathBuf;
	/// use serde::Serialize;
	///
	/// #[derive(Serialize)]
	/// struct Config {
...
/// use configurator::{Configurator, value::magic::{Either, RelativePathBuf, Tagged}};
///
/// #[derive(Debug, PartialEq, Deserialize, Serialize)]
/// struct Config {
...
/// }
///
/// fn configurator<A: Serialize, B: Serialize>(a: A, b: B) -> Configurator {
///     Configurator::from(("int_or_str", a)).merge(("path_or_bytes", b))
/// }
///
/// let config: Config = configurator(10, "/a/b").extract().unwrap();
/// assert_eq!(config.int_or_str, Either::Left(10.into()));
/// assert_eq!(config.path_or_bytes, Either::Left("/a/b".into()));
///
/// let config: Config = configurator("hi", "c/d").extract().unwrap();
/// assert_eq!(config.int_or_str, Either::Right("hi".into()));
/// assert_eq!(config.path_or_bytes, Either::Left("c/d".into()));
///
/// let config: Config = configurator(123, &[1, 2, 3]).extract().unwrap();
/// assert_eq!(config.int_or_str, Either::Left(123.into()));
/// assert_eq!(config.path_or_bytes, Either::Right(vec![1, 2, 3].into()));
///
/// let config: Config = configurator("boo!", &[4, 5, 6]).extract().unwrap();
/// assert_eq!(config.int_or_str, Either::Right("boo!".into()));
/// assert_eq!(config.path_or_bytes, Either::Right(vec![4, 5, 6].into()));
///
/// let config: Config = Configurator::from(configurator::providers::Serialized::defaults(Config {
	...
	B: Deserialize<'b>,
{
	...
	{
		use crate::config::value::ValueVisitor;
...
///
/// ```rust
/// use configurator::{Configurator, value::magic::Tagged, Jail};
/// use configurator::providers::{Format, Toml};
///
/// #[derive(Debug, PartialEq, serde::Deserialize)]
/// struct Config {
...
///     let configurator = Configurator::from(Toml::file("Config.toml"));
///     let c: Config = configurator.extract()?;
///     assert_eq!(*c.number, 10);
///
///     let tag = c.number.tag();
///     let metadata = configurator.get_metadata(tag).expect("number has tag");
///
		...
		if let Some(dict) = de.value.as_dict() {
			if let Some(tagv) = dict.get(Self::FIELDS[0]) {
				if let Ok(false) = tagv.deserialize::<Tag>().map(|t| t.is_default()) {
					return visitor
						.visit_map(MapDe::new(dict, |v| ConfiguredValueDe::<I>::from(config, v)));
				}
			}
		}
...
impl<T> Tagged<T> {
	...
	/// leaf and was extracted from a [`Configurator`](crate::Configurator), the returned
	/// value is expected to be `Some`.
	///
	/// # Example
	///
	/// ```rust
	/// use configurator::{Configurator, Profile, value::magic::Tagged};
	///
	/// let configurator = Configurator::from(("key", "value"));
	/// let tagged = configurator.extract_inner::<Tagged<String>>("key").unwrap();
	///
	...
	///
	/// ```rust
	/// use configurator::{Configurator, value::magic::Tagged};
	///
	/// let tagged = Configurator::from(("key", "value"))
...
mod tests {
	...
	fn test_relative_path_buf() {
		...
		use crate::config::providers::{Format, Toml};
		use std::path::Path;

		crate::config::Jail::expect_with(|jail| {
			...
			let path: RelativePathBuf = Configurator::new()
			...
			let testing: Testing = Configurator::new()
				.merge(Toml::file("Config.toml").nested())
				.select("debug")
				.extract()?;
	...
	//
	//     let profile: SelectedProfile = Configurator::new().extract().unwrap();
	//     assert_eq!(&*profile, crate::Profile::default());
	//
	//     let profile: SelectedProfile = Configurator::new().select("foo").extract().unwrap();
	//     assert_eq!(&*profile, "foo");
	//
	//     let profile: SelectedProfile = Configurator::new().select("bar").extract().unwrap();
	...
	//     }
	//
	//     let testing: Testing = Configurator::from(("value", 123))
	//         .merge(("other", "hi"))
	...
	//     }
	//
	//     let testing: Testing = Configurator::from(("value", 123)).extract().unwrap();
	//
	...
	fn test_tagged() {
		...
		let val = Configurator::from(("foo", "hello"))
		...
		let val = Configurator::from(("bar", "hi"))
