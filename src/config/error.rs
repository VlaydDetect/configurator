...
use crate::config::{Configurator, Metadata, Profile, value::Tag};
...
impl Error {
	...
	pub(crate) fn resolved(mut self, config: &Configurator) -> Self {
...
impl Error {
	...
	///
	/// ```rust
	/// use configurator::config::error::{Error, Kind};
	///
	...
	pub fn with_path(mut self, path: &str) -> Self {
		let paths = path.split('.').filter(|v| !v.is_empty()).map(|v| v.to_string());
	...
	///     "#)?;
	///
	///     let figment = Configurator::from(Toml::file("Base.toml"))
