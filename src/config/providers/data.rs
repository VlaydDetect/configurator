...
use crate::config::value::{Map, Dict};
use crate::config::{Error, Profile, Provider, Metadata};
...
/// use configurator::config::providers::{Format, Data, Json};
///
...
impl<F: Format> Data<F> {
    ...
    /// use configurator::config::{Configurator, Jail, providers::{Format, Toml}, value::Map};
    ///
    ...
    ///         numbers: vec![1, 2, 3],
    ///         untyped: configurator::config::util::map!["key".into() => 1, "other".into() => 4],
    ///     });
    ///
    ///     Ok(())
    /// });
    ...
    ///     "#;
    ///
    ///     let config: Config = Configurator::from(Toml::string(source)).extract()?;
    ///     assert_eq!(config, Config {
    ///         numbers: vec![1, 2, 3],
    ///         untyped: configurator::config::util::map!["key".into() => 1, "other".into() => 4],
    ///     });
    ...
    ///     let figment = Configurator::from(Toml::file("Config.toml").nested());
    ///
    ///     let figment = figment.select("staging");
    ///     let config: Config = figment.extract()?;
    ///     assert_eq!(config, Config {
    ///         numbers: vec![1, 2, 3],
    ///         untyped: configurator::config::util::map!["global".into() => 0, "hi".into() => 7],
    ///     });
    ///
    ...
    /// ```rust
    /// use serde::Deserialize;
    /// use configurator::config::{Configurator, Jail, providers::{Format, Toml}};
    ///
    /// #[derive(Debug, PartialEq, Deserialize)]
    /// struct Config {
    ///     #[serde(default)]
    ...
    ///     let config = Configurator::from(Toml::file("missing.toml")).extract::<Config>()?;
    ///     assert_eq!(config.foo, 0);
    ///
    ///     // Set `required` to true to disallow missing files.
    ///     let source = Toml::file("missing.toml");
    ///     let config = Configurator::from(source.required(true)).extract::<Config>();
    ///     assert!(config.is_err());
    ///
    ///     // Set `required` to false to explicitly allow missing files.
    ///     # let source = Toml::file("missing.toml").required(true);
    ///     let config = Configurator::from(source.required(false)).extract::<Config>()?;
    ///     assert_eq!(config.foo, 0);
    ///
    ///     // The setting has no effect when the file is present.
    ///     let config = Configurator::from(Toml::file("config.toml")).extract::<Config>()?;
    ...
    ///     let config = Configurator::from(source).extract::<Config>()?;
    ///     assert_eq!(config.foo, 123);
    ///
    ...
    ///     let config: Config = Configurator::from(provider).select("debug").extract()?;
...
///
/// ```rust
/// use configurator::config::providers::Format;
///
...
macro_rules! impl_format {
    ...
    ($name:ident $NAME:literal/$string:literal: $func:expr, $E:ty) => (
        impl_format!($name $NAME/$string: $func, $E, concat!(
            ...
            "\n```\n",
            "use configurator::config::providers::{Format, ", stringify!($name), "};",
...
impl YamlExtended {
    ...
    /// ```rust
    /// use serde::Deserialize;
    /// use configurator::config::{Configurator, Jail, providers::{Format, Yaml, YamlExtended}};
    ///
    /// #[derive(Debug, PartialEq, Deserialize)]
    /// struct Circle {
    ///     x: usize,
    ...
    ///     "#)?;
    ///
    ///     let config: Config = Configurator::from(YamlExtended::file("Config.yaml")).extract()?;
    ///     assert_eq!(config, Config {
    ...
    ///     let config = Configurator::from(Yaml::file("Config.yaml"));
