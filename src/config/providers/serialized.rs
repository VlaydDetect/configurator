...
use crate::config::error::{Error, Kind::InvalidType};
use crate::config::value::{Value, Map, Dict};
...
impl<T> Serialized<T> {
    ...
    /// use configurator::{Configurator, Jail, providers::Serialized, config::util::map};
    ///
    ...
    ///     let figment = Configurator::from(Serialized::from(&map, "default"));
    ///     let config: Config = figment.extract()?;
    ///     assert_eq!(config, Config { numbers: vec![1, 2, 3] });
    ///
    ///     // This is also `Serialized::defaults(&map).profile("debug")`;
    ///     let figment = Configurator::from(Serialized::from(&map, "debug"));
    ...
    ///
    /// ```rust
    /// use configurator::{Configurator, Jail, providers::Serialized};
    ///
    /// Jail::expect_with(|jail| {
    ///     // This is also `Serialized::defaults(&map)`;
...
impl<T: Serialize> Provider for Serialized<T> {
    ...
    fn data(&self) -> Result<Map<Profile, Dict>, Error> {
        ...
        let dict = match &self.key {
            Some(key) => crate::config::util::nest(key, value).into_dict().ok_or(error)?,
