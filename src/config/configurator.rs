...
use crate::config::{Profile, Provider, Metadata};
use super::error::{Kind, Result};
use crate::config::value::{Value, Map, Dict, Tag, ConfiguredValueDe, DefaultInterpreter, LossyInterpreter};
use crate::config::coalesce::{Coalescible, Order};

/// Combiner of [`Provider`]s for configuration value extraction.
///
/// # Overview
///
/// A `Configurator` combines providers by merging or joining their provided data.
...
/// in _profiles_, and a profile can be selected via [`Configurator::select()`] for
/// extraction; the profile to be extracted can be retrieved with
/// [`Configurator::profile()`] and defaults to [`Profile::Default`]. The [top-level
...
/// [`adjoin`]: Configurator::adjoin()
/// [`merge`]: Configurator::merge()
/// [`admerge`]: Configurator::admerge()
///
/// For examples, refer to each strategy's documentation.
///
/// ## Extraction
///
/// The configuration or a subset thereof can be extracted from a `Configurator` in
/// one of several ways:
///
///   * [`Configurator::extract()`], which extracts the complete value into any `T:
///     Deserialize`.
///   * [`Configurator::extract_inner()`], which extracts a subset of the value for a
///     given key path.
///   * [`Configurator::find_value()`], which returns the raw, serialized [`Value`]
...
/// ## Metadata
///
/// Every value collected by a `Configurator` is accompanied by the metadata produced
...
///
///   * [`Configurator::metadata()`], which returns an iterator over all of the
///     metadata for all values.
///   * [`Configurator::find_metadata()`], which returns the metadata for a value at
///     a given key path.
///   * [`Configurator::get_metadata()`], which returns the metadata for a given
...
impl Configurator {
    /// Creates a new `Configurator` with the default profile selected and no
    /// providers.
    ///
    /// ```rust
    ...
    /// Creates a new `Configurator` with the default profile selected and an initial
    /// `provider`.
    ///
    /// ```rust
    /// use configurator::Configurator;
    /// use configurator::providers::Env;
    ///
    /// let figment = Configurator::from(Env::raw());
    /// # assert_eq!(figment.profile(), "default");
    /// assert_eq!(figment.metadata().count(), 1);
    /// ```
    #[track_caller]
    pub fn from<T: Provider>(provider: T) -> Self {
        Configurator::new().merge(provider)
    }
    ...
    ///
    /// let new_figment = Configurator::new()
    ...
    /// use configurator::Configurator;
    ///
    /// let figment = Configurator::new().select("staging");
    ...
    /// Returns a new `Configurator` containing only the sub-dictionaries at `key`.
    ///
    ...
    ///
    /// ```rust
    /// use configurator::{Configurator, providers::{Format, Toml}};
    ///
    /// configurator::Jail::expect_with(|jail| {
    ...
    ///     "#)?;
    ///
    ///     let root = Configurator::from(Toml::file("Config.toml"));
    ...
    pub fn focus(&self, key: &str) -> Self {
        fn try_focus(figment: &Configurator, key: &str) -> Result<Map<Profile, Dict>> {
    ...
    ///
    /// use configurator::{Configurator, providers::{Format, Toml, Json, Env}};
    ///
    ...
    ///     "#)?;
    ///
    ///     let config: Config = Configurator::new()
    ...
    /// As [`extract`](Configurator::extract_lossy), but interpret numbers and
    ...
    ///
    /// ```rust
    /// use configurator::{Configurator, providers::{Format, Toml, Json}};
    ///
    /// configurator::Jail::expect_with(|jail| {
    ///     jail.create_file("Config.toml", r#"
    ///         numbers = [1, 2, 3, 10]
    ...
    ///
    ///     let numbers: Vec<usize> = Configurator::new()
    ///         .merge(Toml::file("Config.toml"))
    ///         .join(Json::file("Config.json"))
    ...
    // allow combining `Configurator`s.
    ...
    ///
    /// ```
    /// use configurator::{Configurator, providers::Serialized};
    ///
    /// let figment = Configurator::new();
...
fn is_send_sync() {
    ...
    check_for_send_sync::<Configurator>();
}